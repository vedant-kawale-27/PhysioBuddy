 ...
 * 
 * Provides platform-wide management for superusers:
 * - Aggregated metric cards (Hospitals, Doctors, Patients, Exercises, Assignments)
 * - Registered hospital inspection table with deep-dive modal
 * - Direct links to System Status, Hospitals, and Exercise Library
 * - Quick shortcut to the native Django Admin Panel
 */
export default function SuperAdminDashboard() {
  // Stats state: holds high-level metrics and hospital lists fetched from backend
  const [stats, setStats] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Hospital modal state for inspecting linked doctors and patients
  const [selectedHospital, setSelectedHospital] = useState(null);
  const [hospitalDetail, setHospitalDetail] = useState(null);
  const [loadingDetail, setLoadingDetail] = useState(false);
  const hospitals = stats?.recent_hospitals || stats?.hospitals || [];

  // Load platform statistics on initial component mount
  useEffect(() => {
    fetchDashboard();
  }, []);

  /**
   * Fetches high-level statistics and registered hospital list from Django REST endpoint
   */
  ...
  /**
   * Fetches detailed data for a specific hospital (including list of doctors and patients)
   * @param {number} hospitalId - Database ID of the selected hospital
   */
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50 to-indigo-50 dark:from-gray-950 dark:via-gray-900 dark:to-slate-900 text-gray-900 dark:text-gray-100 font-[Inter]">
      {/* Top Navigation Bar with Super Admin Role */}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {stats && (
          <>
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
              ...
              {/* Total Prescribed Assignments */}
              <div className="backdrop-blur-xl bg-white/70 dark:bg-gray-900/70 border border-white/40 dark:border-gray-800 rounded-2xl p-5 shadow-lg col-span-2 lg:col-span-1">
                <div className="flex items-center justify-between text-gray-500 dark:text-gray-400 mb-2">
            ...
            <div className="backdrop-blur-xl bg-white/80 dark:bg-gray-900/80 border border-white/50 dark:border-gray-800 rounded-3xl p-6 sm:p-8 shadow-xl mb-8">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Click "Inspect Staff & Patients" on any hospital to inspect its associated doctors and patients.
                  </p>
                </div>
                <Link
                  to="/super-admin/hospitals"
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800/60">
                      {hospitals.map((h) => (
                        <tr key={h.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/40 transition">
                          ...
                          <td className="py-4 px-4 text-center">
                            <span className="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-cyan-100 dark:bg-cyan-900/40 text-cyan-700 dark:text-cyan-300">
                              {h.doctors_count ?? h.doctor_count ?? 0}
                            </span>
                          </td>
                          <td className="py-4 px-4 text-center">
                            <span className="inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300">
                              {h.patients_count ?? h.patient_count ?? 0}
                            </span>
                          </td>
                          <td className="py-4 px-4 text-xs text-gray-500 dark:text-gray-400">
                            {h.created_at ? h.created_at.split('T')[0] : 'N/A'}
                          </td>
                          <td className="py-4 px-4 text-right">
                            <button
                              onClick={() => viewHospitalDetails(h.id)}
                              className="px-3.5 py-1.5 rounded-xl bg-cyan-500/10 hover:bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 font-bold text-xs transition cursor-pointer"
                            >
        ...
        {/* Hospital Details Modal: Shows list of linked doctors and patients */}
        {selectedHospital && (
